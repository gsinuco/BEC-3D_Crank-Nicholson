...
MODULE physical_constants
  ...
  DOUBLE PRECISION, PARAMETER:: deltat  = 1.0E-6
...
!--------------------------------------------------------------------------------------------------------------------------
!s_m: large vector of how many elements each procesor has to send (send matric) 0:0, 0:1.... 1:0,1:1,1:2 ....
!s_c: vector of how many elements each procesor sent (sent counter): i:0,i:1,i:2....
...
!--------- VARIABLES AND CONSTANTS FOR MPI -------------
  ...
  INTEGER, DIMENSION(:), ALLOCATABLE    :: i_prima,i_message,i_abs,i_abs_
  INTEGER old_proc,new_proc
  INTEGER i,j,k,l,s,i_,s_
  INTEGER n_p_p

  CHARACTER*40 file_name
  CHARACTER*40 file_name2
  CHARACTER*10 sub
...
!------------------------------------------------------------------------
  ...
  write(sub,'(i2)') myproc
  file_name ='test_te_grid_'//adjustl(trim(sub))
  file_name = adjustl(file_name)

  file_name2 = 'datos_P'//adjustl(trim(sub))
  ...
  beta(1)  = DCMPLX(0.0,0.25*deltat*hbar/(mass*(DELTA(1)**2)))
  beta(2)  = DCMPLX(0.0,0.25*deltat*hbar/(mass*(DELTA(2)**2)))
  beta(3)  = DCMPLX(0.0,0.25*deltat*hbar/(mass*(DELTA(3)**2)))
  alpha    = DCMPLX(0.0,-deltat/(2*D*hbar))

!  IF(myproc.eq.0) THEN
!     open(111,file="sorted_index.dat",action="read")  
!     open(110,file="sorted_index2.dat",action="write")
!  END IF	
...
!     close(110)
  ...
  DO channels=0,0
     ...
     DO r = 1,iterations
        ...
        IF(MOD(r,iterations/frames) .EQ. 0) THEN
           IF(myproc.EQ.0) write(*,*) 'I am writing'
           CALL WRITE_FUN(x_,delta,BOX_SIZE,N,n_p_p,file_name)
        END IF
...
SUBROUTINE init_parallel_com(s_m,r_m,i_prima,i_abs,i_message,numprocs,NN,s,N)
    ...
    !INTEGER, DIMENSION(NN) :: i_message_
    INTEGER old_proc,new_proc
    INTEGER i,j,k,l
    ...
    s_m = 0
     ...
     DO i=1,NN
       ...
       END IF
        ...
        !write(100,109)i,j,k,l,i_prima(i),old_proc(i),new_proc(i),(old_proc(i)+1-1)*numprocs + new_proc(i) + 1,i_abs(i)
     ...
     DO i=1,numprocs
        DO j=1,numprocs
           ...
           !write(100,*)  i,j, s_m((i-1)*numprocs +j),r_m((j-1)*numprocs +i)
        END DO
        !write(100,*)
     END DO

     !     CALL sorting(i_prima,s_m,n_p_p,i_message,NN,N,numprocs,s,index_)	
     n_x = N(1)/numprocs
     n_y = N(2)/numprocs
     n_z = N(3)/numprocs
...
SUBROUTINE init_parallel_local(i_abs_,s_i,s_c,r_c,i_abs_sent,s_d,r_d,s,numprocs,n_p_p)
    ...
    DO i=2,numprocs
       ...
       ! write(100,106) myproc,i-1,s_c(s,i),s_d(s,i),r_c(s,i),r_d(s,i)
     END DO
...
SUBROUTINE INITIAL_STATE(x,delta,w,BOX_SIZE,N)
  ...
  open(2,file="gs3D_test_paralelo.dat",action="read")

  length_scale(1) = SQRT(4.0*hbar/(mass*w(1)))
  length_scale(2) = SQRT(4.0*hbar/(mass*w(2)))
  length_scale(3) = SQRT(4.0*hbar/(mass*w(3)))
 ...
 i_= ((l)-1)*N(1)*N(2) + ((k)-1 + 0)*N(1) + (j+0) !para desplazar el estado inicial
	read(2,201) partereal
	if(i_.LE.NN)	x(i_,1) = DCMPLX(partereal,0.0)
...
!   open(4,file="energy_test_2.dat",action="read")
! 
!   DO i=1,NN
!      read(4,201) energy
