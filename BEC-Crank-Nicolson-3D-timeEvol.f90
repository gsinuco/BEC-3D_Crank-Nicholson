...
MODULE geometrical_constants
  INTEGER, PARAMETER :: iterations = 27000
  INTEGER, PARAMETER :: frames     = 1000
  ...
  DOUBLE PRECISION, PARAMETER:: BOX_SIZE_X = 120.0E-6
  DOUBLE PRECISION, PARAMETER:: BOX_SIZE_Y = 8.0E-6
  DOUBLE PRECISION, PARAMETER:: BOX_SIZE_Z = 8.0E-6
...
MODULE physical_constants
  ...
  DOUBLE PRECISION, PARAMETER:: deltat  = 2.5E-6
...
program tridiagonal      
  ...
  INTEGER          :: N(3)
  DOUBLE PRECISION :: DELTA(3),BOX_SIZE(3),w(3),time
  ...
  CALL EXTERNAL_POTENTIAL(V_ext,time, delta,w,BOX_SIZE,N)
...
SUBROUTINE INITIAL_STATE(x,delta,w,BOX_SIZE,N)
  ...
  INTEGER          :: i,j,k,l, N_med(3),i_
  DOUBLE PRECISION :: x_j,y_k,z_l,w_bar,a_bar,mu_TF,partereal
  DOUBLE PRECISION :: sigma2(3),length_scale(3)

  open(4,file="gs3D_test_2.dat",action="read") 
  
  N_med(3) = INT(0.5*N(3))
 
  length_scale(1) = SQRT(60.0*hbar/(mass*w(1)))
  ...
  DO i=1,NN
     ...
     i_= (l+0)*N(1)*N(2) + (k-0)*N(1) + (j+0) !para desplazar el estado inicial
     read(4,201) partereal
...
SUBROUTINE EXTERNAL_POTENTIAL(V_ext,time,delta,w,BOX_SIZE,N)
  USE physical_constants
  USE geometrical_constants
  USE potential_constants
  USE atomic_properties
  ...
  IMPLICIT NONE
  INTEGER,INTENT (IN) :: N(3)
  DOUBLE PRECISION, INTENT (IN)  :: delta(3),w(3),BOX_SIZE(3),time
  COMPLEX*16, INTENT (OUT) :: V_ext(NN)

  INTEGER          :: i,j,k,l
  DOUBLE PRECISION :: x_j,y_k,z_l,energy

  !open(2,file="energy_test_2.dat",action="read")
  ...
  DO i=1,NN
     ...
     k = (i-j-(l-1)*(N(1)*N(2)))/N(1) + 1
      ...
      V_ext(i) = DCMPLX(0.5*mass*(((w(1)*cos(2*w(1)*time))*x_j)**2  + (w(2)*y_k)**2 + (w(3)*z_l)**2),0.0)   
...
SUBROUTINE WRITE_FUN(x_aux,delta,BOX_SIZE)
  ...
  INTEGER :: i,j,k,l
  DOUBLE PRECISION :: density_1D(N_x),density_2D(N_x,N_y)
  DOUBLE PRECISION :: x_j,y_k
  
  open(2,file="te1D_test_2_serieV2.dat",ACCESS="append")
  open(3,file="te3D_test_2.dat",ACCESS="append")
...
!!$     write(2,203) 1E6*x_j,1E6*y_k,N_0*density_2D(j,k)/1E12
!!$  END DO
!!$  WRITE(2,*)
!!$  END DO
!!$
